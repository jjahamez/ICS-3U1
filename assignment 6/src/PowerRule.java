...
public class PowerRule {
    public static void main(String[] args) throws Exception {
        ...
        int[] exp2 = new int[exp1.length]; // stores number for 2nd derivative
        ...
        try {
            for (int i = 0; i < coeff1.length; i++) { // Loop repeats(based on the # of coefficients)
                ...
                if (exponent > 0) {
                    deriv1 += (coefficient > 0 && deriv1.length() > 7 ? " +" : " ") + coefficient + "x^" + exponent; // formats term >7 as it ignores the beginning & 1st term
                    exp2[i] = exponent;
            ...
            for (int i = 0; i < coeff2.length; i++) { // Loop repeats(based on the # of exponents)
                ...
                int exponent = exp2[i] - 1; // Reduces exponent by 1 to apply power rule 
                if (exponent > 0) {
