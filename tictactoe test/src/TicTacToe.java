import java.util.*; //could just be scanner 
...
public class TicTacToe {
   ...
   public static void main(String[] args) {
      ...
      System.out.println("Enter your move as an integer from 1-9 (refer to grid below) ");
      ...
      while (true) {
         ...
         try { // reads as string and converts to int, then -1 to match index
            move = Integer.parseInt(input.nextLine()) - 1; // string to avoid edge case of 1 2 (instead of
                                                           // input.nextInt)
            int row = move / 3;
            ...
            board[row][col] 
            = currentPlayer.charAt(0); // updates board to 'X' or 'O' (1st string to char)
            moveCount++;
